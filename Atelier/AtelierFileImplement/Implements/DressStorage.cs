...
namespace AtelierFileImplement.Implements
{
    public class DressStorage : IDressStorage
    {
        ...
        {
            return source.Dresses.Select(CreateModel).ToList();
        }

        public List<DressViewModel> GetFilteredList(DressBindingModel model)
        {
            if (model == null) return null;
            return source.Dresses.Where(rec => rec.DressName.Contains(model.DressName))
        ...
        public void Insert(DressBindingModel model)
        {
            ...
            var element = new Dress { Id = maxId + 1, DressComponents = new Dictionary<int, int>() };
        ...
        public void Delete(DressBindingModel model)
        {
            ...
            if (element != null) source.Dresses.Remove(element);
        ...
        private static Dress CreateModel(DressBindingModel model, Dress dress)
        {
            ...
            foreach (var component in model.DressComponents)
            {
                if (dress.DressComponents.ContainsKey(component.Key))
                    dress.DressComponents[component.Key] = model.DressComponents[component.Key].Item2;
                else
                    dress.DressComponents.Add(component.Key, model.DressComponents[component.Key].Item2);
            }
            return dress;
        }
